...
def replay(symbol: str, bars: list[dict]):
    """Roll-forward: für jeden Bar ab Index 12 detect_bull_flag aufrufen."""
    if len(bars) < 15:
        print(f"  {symbol}: nur {len(bars)} bars, skip")
        return []
    triggers = []
    for i in range(12, len(bars)):
        window = bars[:i+1]
        ok, params = bot_mod.detect_bull_flag(window)
        if ok:
            ts = bars[i]["ts"]
            triggers.append({
                "time": ts,
...
def main():
    ...
    for sym, bars in bars_by_sym.items():
        ...
        if not triggers:
            print(f"  no triggers")
            continue
        for t in triggers[:5]:  # erste 5 Trigger pro Symbol
        ...
        if len(triggers) > 5:
            print(f"  ... + {len(triggers)-5} more triggers")
